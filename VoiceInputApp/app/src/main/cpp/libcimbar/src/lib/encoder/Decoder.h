...
public:
	...
	inline static uint64_t readerInitMs = 0;
	inline static uint64_t symbolReadMs = 0;
	inline static uint64_t symbolFlushMs = 0;
	inline static uint64_t ccmMs = 0;
	inline static uint64_t colorReadMs = 0;
	inline static uint64_t colorFlushMs = 0;
...
inline unsigned Decoder::do_decode(CimbReader& reader, STREAM& ostream)
{
...

	{
		...
		symbolReadMs += std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - phaseBegin).count();

		// flush symbols
		phaseBegin = std::chrono::steady_clock::now();
		reed_solomon_stream rss(ostream, eccBytes, eccBlockSize);
		symbolBuff.flush(rss);
		symbolFlushMs += std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - phaseBegin).count();
	}
	...
	ccmMs += std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - phaseBegin).count();
	...
	colorReadMs += std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - phaseBegin).count();

	phaseBegin = std::chrono::steady_clock::now();
	reed_solomon_stream rss(ostream, eccBytes, eccBlockSize);
	// flush() will return the (good) cumulative bytes written to the underlying stream
	unsigned res = colorBuff.flush(rss);
	colorFlushMs += std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - phaseBegin).count();
	return res;
}
...
inline unsigned Decoder::decode_fountain(const MAT& img, FOUNTAINSTREAM& ostream, bool should_preprocess, int color_correction)
{
	...
	CimbReader reader(img, _decoder, cimbar::Config::color_mode(), should_preprocess, color_correction);
	readerInitMs += std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - begin).count();
