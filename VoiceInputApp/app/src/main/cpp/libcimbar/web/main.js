var Main = function () {
  ...
  var _interval = 22;
  var _fps = 45;
  var _rampTargetFps = 0;
  var _rampStartFps = 45;
  var _rampStartTime = 0;
  var _rampDuration = 0;
  ...
  function applyJitter(canvas) {
    canvas.style.transform = "";
  }

  function clampFPS(val) {
    return Math.max(5, Math.min(60, Number(val)));
  }
  ...
  function currentFPS() {
    ...
    var elapsed = performance.now() - _rampStartTime;
    var progress = Math.min(1, elapsed / _rampDuration);
    var fps = _rampStartFps + ((_rampTargetFps - _rampStartFps) * progress);
    applyFPS(fps);
    if (progress >= 1) {
      _rampTargetFps = 0;
    }
  ...
  function updateFPSLabel() {
    var label = Math.round(_fps) + " fps";
    if (_rampTargetFps) {
      label += " -> " + Math.round(_rampTargetFps);
    }
    Main.setHTML("fps-value", label);
  }
  ...
  return {
    ...
    nextFrame: function () {
      ...
      if (!Main.isPaused()) {
        ...
        applyJitter(document.getElementById('canvas'));
      }
    ...
    rampFPS: function (target) {
      target = clampFPS(target || 60);
      currentFPS();
      _rampStartFps = _fps;
      _rampTargetFps = target;
      _rampStartTime = performance.now();
      _rampDuration = Math.max(1000, Math.abs(_rampTargetFps - _rampStartFps) * 1000);
      var slider = document.getElementById("fps-slider");
      if (slider) {
        slider.value = target;
      }
    ...
    setJitter: function (val) {
      Main.setHTML("jitter-value", "原版");
      var slider = document.getElementById("jitter-slider");
      if (slider) {
        slider.value = 0;
      }
    },
